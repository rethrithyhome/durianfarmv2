...
/** Upsert on (worker, date) — re-recording the same day corrects the
 * hours instead of creating a duplicate entry. */
...
interface PaymentRow {
  id: string; worker_id: string; wage_type: WageType; hours: number | null;
...
const payFromRow = (r: PaymentRow): PayrollPayment => ({
  id: r.id, workerId: r.worker_id, wageType: r.wage_type ?? "monthly", hoursPaid: r.hours == null ? null : Number(r.hours),
  cycleStart: r.cycle_start, cycleEnd: r.cycle_end,
...
const payToRow = (p: Partial<PayrollPayment>, farmId: string) => ({
  farm_id: farmId, worker_id: p.workerId, wage_type: p.wageType ?? "monthly", hours: p.hoursPaid ?? null,
  cycle_start: p.cycleStart, cycle_end: p.cycleEnd,
...
/**
 * Records a payment and, for hourly workers, stamps the work-log rows it
 * covers so those days can't be paid a second time. Any number of
 * payouts per month is fine — the days themselves are what's tracked.
 */
export async function createPayrollPayment(
  p: Partial<PayrollPayment>,
  workLogIds: string[] = [],
  farmId: string = DEFAULT_FARM_ID
): Promise<PayrollPayment> {
  const payment = payFromRow(must(await supabase.from("payroll_payments").insert(payToRow(p, farmId)).select().single<PaymentRow>()));
  if (workLogIds.length > 0) {
    const res = await supabase.from("work_logs").update({ payment_id: payment.id }).in("id", workLogIds);
    if (res.error) throw res.error;
  }
  return payment;
}

/** Deleting a payment releases the days it covered back to unpaid. */
export async function deletePayrollPayment(id: string): Promise<void> {
  const release = await supabase.from("work_logs").update({ payment_id: null }).eq("payment_id", id);
