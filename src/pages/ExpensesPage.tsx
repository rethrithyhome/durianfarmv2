...
import type { Expense, Role } from "@/types/domain";
...
export function ExpensesPage({ role }: { role: Role }) {
  ...
  const sorted = useMemo(() => {
    ...
    else if (sort === "amount") list.sort((a, b) => b.amount - a.amount);
  ...
  return (
    <div className="pt-1 pb-4">
      <div className="flex items-center justify-between mb-3">
        <div className="text-sm font-semibold" style={{ color: C.green }}>ចំណាយសរុប៖ {fmtMoney(total)}</div>
